...
describe('CreditCardPurchasesService', () => {
  it('rejects a card from another user as not found', async () => {
    ...
    await assert.rejects(() => new CreditCardPurchasesService(prisma as any).create('user-a', dto), NotFoundException);
  });
  ...
  it('rejects a card without closingDay', async () => {
    ...
    await assert.rejects(() => new CreditCardPurchasesService(prisma as any).create('user-a', dto), BadRequestException);
  });
  ...
  it('rejects a category that is absent, foreign, or not an expense', async () => {
    ...
    await assert.rejects(() => new CreditCardPurchasesService(prisma as any).create('user-a', { ...dto, categoryId: '00000000-0000-4000-8000-000000000002' }), BadRequestException);
  });
  ...
  it('reuses the same monthly invoice for multiple purchases', async () => {
    ...
    const service = new CreditCardPurchasesService(prisma as any);
