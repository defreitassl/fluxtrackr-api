...
export class SubscriptionChargesService {
  ...
  realize(userId: string, id: string, dto: RealizeSubscriptionChargeDto) {
    ...
    if (Number.isNaN(occurredAt.getTime())) throw new BadRequestException('occurredAt must be a valid ISO date');
    ...
    return this.runSerializableTransaction(async (tx) => {
      ...
      const accountOverride = dto.accountId !== undefined;
      const creditCardOverride = dto.creditCardId !== undefined;
      const accountId = accountOverride ? dto.accountId : creditCardOverride ? null : charge.accountId ?? charge.subscription.accountId;
      const creditCardId = creditCardOverride ? dto.creditCardId : accountOverride ? null : charge.creditCardId ?? charge.subscription.creditCardId;
      const categoryId = dto.categoryId ?? charge.categoryId ?? charge.subscription.categoryId;
      const paymentMethod = creditCardOverride ? null : dto.paymentMethod ?? charge.paymentMethod ?? charge.subscription.paymentMethod;
      await this.validateResolved(tx, userId, accountId, creditCardId, categoryId, paymentMethod);
      const realizedAt = now;
      if (accountId) {
