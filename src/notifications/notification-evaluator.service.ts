...
export class NotificationEvaluatorService implements OnApplicationBootstrap {
  ...
  async evaluateUser(userId: string, asOf: Date, client: Db = this.prisma) {
    ...
    for (const row of invoices) await this.evaluateInvoice(userId, row.id, asOf, client);
    for (const row of events) await this.evaluateFinancialEvent(userId, row.id, asOf, client);
    for (const row of charges) await this.evaluateSubscriptionCharge(userId, row.id, asOf, client);
    for (const row of budgets) await this.evaluateBudget(userId, row.id, asOf, client);
    for (const row of goals) await this.evaluateGoal(userId, row.id, asOf, client);
  }
  ...
  async evaluateFinancialEvent(userId: string, eventId: string, asOf: Date, client: Db = this.prisma) {
    ...
    if (!active) return this.notifications.resolveSource(client, userId, 'financial_event', eventId, [type], asOf);
    if (!pref.enabled) return;
    return this.activate(client, userId, { category: 'events', type, severity: 'info', title: 'Evento financeiro próximo', message: `${event.name} está previsto para ${dateKey(event.date)}.`, sourceType: 'financial_event', sourceId: event.id, dedupeKey: `event_upcoming:${event.id}:${dateKey(event.date)}`, scheduledFor: event.date });
  }
  async evaluateSubscriptionCharge(userId: string, chargeId: string, asOf: Date, client: Db = this.prisma) {
    ...
    if (!overdue && !upcoming) return this.notifications.resolveSource(client, userId, 'subscription_charge', chargeId, [...types], asOf);
    const type = overdue ? 'subscription_charge_overdue' : 'subscription_charge_upcoming';
    await this.notifications.resolveSource(client, userId, 'subscription_charge', chargeId, types.filter((item) => item !== type) as any, asOf);
    if (!pref.enabled) return;
  ...
  async evaluateGoal(userId: string, goalId: string, asOf: Date, client: Db = this.prisma) {
    ...
    const pref = await this.preferences.getEffective(userId, 'goals', client); const current = day(asOf); const overdue = day(goal.targetDate) < current; const upcoming = !overdue && day(goal.targetDate) <= new Date(current.getTime() + (pref.leadDays ?? 30) * 86_400_000); if (!overdue && !upcoming) return this.notifications.resolveSource(client, userId, 'financial_goal', goalId, [...types], asOf); if (!pref.enabled) return; const type = overdue ? 'goal_overdue' : 'goal_deadline_upcoming'; await this.notifications.resolveSource(client, userId, 'financial_goal', goalId, types.filter((item) => item !== type) as any, asOf); return this.activate(client, userId, { category: 'goals', type, severity: overdue ? 'critical' : 'warning', title: overdue ? 'Meta vencida' : 'Prazo da meta próximo', message: `${goal.name} tem prazo em ${dateKey(goal.targetDate)}.`, sourceType: 'financial_goal', sourceId: goal.id, dedupeKey: `${overdue ? 'goal_overdue' : 'goal_deadline_upcoming'}:${goal.id}:${dateKey(goal.targetDate)}`, scheduledFor: goal.targetDate });
  }
